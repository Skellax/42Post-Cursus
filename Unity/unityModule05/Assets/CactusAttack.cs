...
public class CactusAttack : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        negativeX = -cactus.transform.localScale.x;
        positiveX = cactus.transform.localScale.x;
    }
    ...
    private void OnTriggerStay2D(Collider2D collision)
    {
        if (collision.gameObject.CompareTag("Player"))
        {
            ...
            {
                cactus.transform.localScale = new Vector3(positiveX, cactus.transform.localScale.y, cactus.transform.localScale.z);
                speed = -8f;
                isRight = false;
            ...
            {
                cactus.transform.localScale = new Vector3(negativeX, cactus.transform.localScale.y, cactus.transform.localScale.z);
                speed = 8f;
                isRight = true;
            }
            if (!isShooting)
